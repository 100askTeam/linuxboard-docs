...
本章节将讲解如何在 T153MX 多核 Arm 系统中构建和引导同构 AMP 从系统，并通过内存规划、启动日志和 RPMsg 通信验证主从系统协同运行。
...
正文以原始 PDF 为技术依据，并补充 T153MX 同构 AMP 的执行环境、资源边界和验收方法。

:::

## 目标

完成同构从系统的构建、内存隔离、引导和主从通信验证。

## 准备工作

- 明确 Linux 主系统使用的 CPU 核，以及 FreeRTOS/裸机从系统使用的 CPU 核。
- 核对从系统固件入口地址、加载地址、保留内存和通信共享内存。
- 准备独立终端配置 Linux 与 RTOS 构建环境，避免环境变量相互覆盖。
- 保存当前可启动镜像和板级设备树，以便内存或引导配置错误时恢复。

## 操作步骤

1. 构建 FreeRTOS 或裸机从系统并确认固件产物。
2. 配置 Boot0、U-Boot 或 Kernel 中的一种引导方式。
3. 核对主从系统内存区域没有重叠。
4. 重新打包和烧录完整镜像。
5. 从串口日志确认两个系统启动，再执行 RPMsg 通信测试。

## 验证方法与完成标准

主系统正常启动并不代表 AMP 成功。必须同时确认从系统启动日志、CPU/内存隔离、通信端点出现，以及双向测试数据一致。

## T153MX 适用性检查

同构 AMP 会从 Linux SMP 使用的 Arm 核中隔离资源。开始移植前先从芯片规格、板级设备树和当前启动日志共同确认 CPU 拓扑，不能只照搬其他平台的核编号。

| 必须确认 | 项目值 |
| --- | --- |
| Linux 保留的 Arm 核 | `TODO: 待确认` |
| 从系统使用的 Arm 核 | `TODO: 待确认` |
| 从系统加载地址和入口 | `TODO: 待确认` |
| Linux 保留内存 | `TODO: 待确认` |
| vring/共享内存 | `TODO: 待确认` |
| Boot0、U-Boot 或 Kernel 引导 | `TODO: 待确认` |

```mermaid
flowchart TD
    A[确认 CPU 与内存拓扑] --> B[构建从系统]
    B --> C[检查 ELF/BIN 地址]
    C --> D[配置一种引导方式]
    D --> E[打包并烧录]
    E --> F[分别确认主从系统日志]
...
如果项目实际目标是 Linux A7 管理 E907 FreeRTOS，应改看[异构通信框架](./04-HeterogeneousCommunication.md)，而不是本篇同构 AMP。
