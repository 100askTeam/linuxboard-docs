...
本章节将讲解如何在 T153MX Tina Linux 中选择 SWUpdate OTA 方案、制作升级包、执行升级并验证版本、分区和回滚状态。
...
正文以原始 PDF 为技术依据，并补充 T153MX OTA 操作的风险边界、输入产物和验证闭环。

:::

## 目标

为 T153MX 选择合适的升级方案，制作可校验升级包，并完成升级、重启和回滚验证。

## 准备工作

- 确认设备采用 Recovery、A/B 整包还是 A/B 差分方案。
- 保存当前可启动的完整固件、分区表、环境变量和重要业务数据。
- 准备稳定电源，升级过程中不得断电或复位。
- 核对升级包适用的板型、存储介质、软件版本和目标分区大小。

## 操作步骤与升级前确认

| 项目 | 必须确认的内容 |
| --- | --- |
| 输入 | 基础版本、目标版本、镜像及签名材料 |
| 产物 | SWUpdate 包、描述文件和校验信息 |
| 目标 | 当前槽位、目标槽位及升级分区 |
| 恢复 | 失败后的回滚条件、串口入口和重新烧录方法 |

OTA 会持久化修改启动介质。首次验证应使用可恢复的测试设备，并保留完整串口日志。

## 验证方法与完成标准

升级工具返回成功后，还必须重启验证目标版本、活动槽位、关键分区校验和业务功能；只有升级命令退出成功但新系统不能稳定启动，不算升级完成。

## T153MX 升级前置门槛

在制作升级包之前，先把下表填完整。任何一项还是 `TODO` 时，都不应直接在业务设备上升级。

| 项目 | 项目实测值 |
| --- | --- |
| 当前可启动镜像及 SHA-256 | `TODO: 待确认` |
| 当前版本与目标版本 | `TODO: 待确认` |
| Recovery 或 A/B 方案 | `TODO: 待确认` |
| 当前活动槽位 | `TODO: 待确认` |
| 升级目标分区与容量 | `TODO: 待确认` |
| 签名/校验策略 | `TODO: 待确认` |
| 断电或校验失败后的恢复路径 | `TODO: 待确认` |

```mermaid
flowchart LR
    A[确认基线和分区] --> B[制作并校验升级包]
    B --> C[测试设备执行升级]
    C --> D[重启进入目标系统]
    D --> E[验证版本、槽位和业务]
    E --> F[故障注入与回滚测试]
...
本手册中的 Recovery、A/B 和差分示例是不同方案，不能同时混用。实际项目采用哪种方案必须由当前板级分区配置和启动控制逻辑共同确认。
